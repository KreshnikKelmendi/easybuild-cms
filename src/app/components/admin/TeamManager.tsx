...
const TeamManager = () => {
  ...
  const [formData, setFormData] = useState<TeamData>({
    title: { en: '', de: '', al: '' },
    firstDescription: { en: '', de: '', al: '' },
    secondDescription: { en: '', de: '', al: '' },
    image: '',
  });
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        setFormData({
          ...
          firstDescription: {
            en: data.data.firstDescription?.en || '',
            de: data.data.firstDescription?.de || '',
            al: data.data.firstDescription?.al || ''
          },
          secondDescription: {
            en: data.data.secondDescription?.en || '',
            de: data.data.secondDescription?.de || '',
            al: data.data.secondDescription?.al || ''
          },
          image: data.data.image || '',
        });
      }
    } catch (_error) {
      console.error('Error fetching current team:', _error);
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name.includes('.')) {
      ...
      setFormData(prev => {
        const currentField = prev[field as keyof TeamData];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setMessage('Team section saved successfully!');
        fetchCurrentTeam();
      } else {
        setMessage(data.message || 'Failed to save team section');
      }
    } catch {
      setMessage('An error occurred while saving the team section');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 bg-white">
      ...
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold text-black mb-1 font-zonapro">Team Manager</h1>
        <p className="text-black font-zonapro">Create and edit your team section content</p>
      </div>
      ...
      <div className="border border-gray-300 p-4 mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-black mb-1 font-zonapro">
              First Description ({languages.find(l => l.code === currentLanguage)?.name})
            </label>
            <textarea
              name={`firstDescription.${currentLanguage}`}
              value={formData.firstDescription[currentLanguage]}
              onChange={handleInputChange}
              rows={4}
          ...
          <div>
            <label className="block text-sm font-medium text-black mb-1 font-zonapro">
              Second Description ({languages.find(l => l.code === currentLanguage)?.name})
            </label>
            <textarea
              name={`secondDescription.${currentLanguage}`}
              value={formData.secondDescription[currentLanguage]}
              onChange={handleInputChange}
              rows={4}
              className="w-full px-3 py-2 border border-gray-400 rounded focus:outline-none focus:border-black text-black resize-none font-zonapro"
          ...
          <div className="flex gap-3 pt-3">
            <button
              ...
              onClick={async () => {
                try {
                  const response = await fetch('/api/seed-team', { method: 'POST' });
                  const data = await response.json();
                  if (data.success) {
                    setMessage('Sample team section created!');
                    fetchCurrentTeam();
                  } else {
                    setMessage(data.message || 'Failed to create sample team section');
                  }
                } catch {
                  setMessage('Error creating sample team section');
                }
              }}
            ...
            >
              {isLoading ? 'Saving...' : 'Save Team Section'}
            </button>
          </div>
        </form>
      </div>
      ...
      {currentTeam && (
        <div className="border border-gray-300 p-4">
          <h3 className="text-lg font-medium text-black mb-3 font-zonapro">Current Team Section</h3>
            ...
            {languages.map((lang) => (
              <div key={lang.code} className="p-3 bg-gray-100 rounded border border-gray-200">
                ...
                <div className="text-sm text-black">
                  <div className="mb-1 font-zonapro"><strong>Title:</strong> {currentTeam.title?.[lang.code] || 'Not set'}</div>
                  <div className="mb-1 font-zonapro"><strong>First Description:</strong> {currentTeam.firstDescription?.[lang.code] || 'Not set'}</div>
                  <div className="font-zonapro"><strong>Second Description:</strong> {currentTeam.secondDescription?.[lang.code] || 'Not set'}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 p-3 bg-gray-100 rounded border border-gray-200">
            <div className="font-medium text-black mb-2 font-zonapro">🖼️ Team Image (Used for all languages)</div>
            <div className="text-sm text-black mb-2 font-zonapro">
              <strong>Image:</strong> {currentTeam.image || 'Not set'}
            </div>
            {currentTeam.image && (
